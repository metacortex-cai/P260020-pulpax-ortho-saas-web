...
function LoginContent() {
  ...
  // Modes: 'login' | 'forgot' | 'reset' | 'invite'
  const [mode, setMode] = useState<'login' | 'forgot' | 'reset' | 'invite'>('login');
  const [resetToken, setResetToken] = useState<string | null>(null);
  const [inviteToken, setInviteToken] = useState<string | null>(null);

  // Derive mode/tokens from the URL query on mount or whenever it changes (React docs:
  // adjust state during render instead of copying a prop into state via an effect).
  const searchParamsKey = searchParams.toString();
  // Starts as `null` (never equal to a real query string) so the very first render
  // still processes a resetToken/inviteToken already present in the URL on load.
  ...
  if (searchParamsKey !== prevSearchParamsKey) {
    ...
    if (token) {
      ...
      const invite = searchParams.get('inviteToken');
      if (invite) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // SaaS admin bu porttan giriş yapamaz
      if (user.role && user.role.startsWith('SAAS_')) {
        throw new Error('Yetkisiz erişim: Lütfen SaaS Admin panelinden (port 6001) giriş yapın.');
      }
  ...
  const handleAcceptInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Davet kabul edilemedi.');
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="p-6">
          ...
          <div className="text-center mb-6 flex flex-col items-center">
            ...
            <h4 className="text-lg font-bold text-white">Pulpax Clinic</h4>
          </div>
          ...
          {/* Mode 4: ACCEPT INVITE */}
          {mode === 'invite' && (
            <div className="space-y-4">
              <h5 className="text-white font-semibold text-center mb-2">Hesabınıza Hoş Geldiniz — Şifrenizi Belirleyin</h5>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/80">Şifre (Tekrar)</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-white/40">
                    <Lock size={18} />
                  </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 text-white font-semibold py-2.5 px-4 rounded-xl shadow-lg active:scale-[0.98] transition-all duration-200 disabled:opacity-70 text-sm btn-success-green"
              >
                {loading ? 'Hesap Aktifleştiriliyor...' : 'Hesabımı Aktifleştir'}
              </button>
            </div>
          )}
        </div>
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-slate-950 text-white">
        <div className="animate-pulse font-bold text-sm tracking-wide uppercase opacity-70">Pulpax Yükleniyor...</div>
      </div>
    }>
      <LoginContent />
    </Suspense>
