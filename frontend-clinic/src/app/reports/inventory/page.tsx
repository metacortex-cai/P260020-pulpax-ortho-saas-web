...
import { ReportsService, InventoryReportItem } from '../../../lib/services/reports.service';
...
export default function InventoryReportPage() {
  ...
  const getStatusCls = (s: string) => {
    ...
    if (s === 'AZALIYOR') return 'bg-amber-50 text-amber-600';
    return 'bg-metronic-success-light text-metronic-success';
  };
  ...
  const criticalCount = data.filter(i => i.status === 'KRİTİK').length;
  ...
  const exportCSV = () => {
    const BOM = '﻿';
    const headers = ['Malzeme No', 'Malzeme Adı', 'Kategori', 'Stok Miktarı', 'Birim', 'Durum', 'Stok Değeri (₺)'];
    const rows = sorted.map(r => [r.id, r.name, r.category, r.stock, r.unit, r.status, r.value.toFixed(2)]);
    const csv = BOM + [headers, ...rows].map(row => row.map(v => `"${String(v).replace(/"/g, '""')}"`).join(';')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'stok_envanter_raporu.csv'; a.click();
    URL.revokeObjectURL(url);
  };

  const statusOpts = [
    { value: 'ALL', label: 'Tüm Durumlar' },
    { value: 'KRİTİK', label: 'Kritik Seviye' },
    { value: 'AZALIYOR', label: 'Azalıyor' },
    { value: 'YETERLİ', label: 'Yeterli' },
  ];

  return (
    <MetronicLayout title="Stok Envanter Raporu" breadcrumbs={['Raporlar', 'Stok']}>
      <style>{`@keyframes fadeInDown { from { opacity:0; transform:translateY(-6px); } to { opacity:1; transform:translateY(0); } }`}</style>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Toplam Stok Değeri', value: loading ? '—' : fmt(totalValue), sub: 'Güncel envanter', icon: <Package size={20} />, color: 'text-metronic-primary', bg: 'bg-metronic-primary-light dark:bg-metronic-primary/10' },
          { label: 'Kritik Seviye', value: loading ? '—' : `${criticalCount} Kalem`, sub: 'Hemen sipariş gerekli', icon: <AlertTriangle size={20} />, color: 'text-metronic-danger', bg: 'bg-red-50 dark:bg-red-500/10' },
          { label: 'Bu Ayki Tüketim', value: '—', sub: 'Kullanılan malzeme', icon: <History size={20} />, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-500/10' },
          { label: 'Toplam Kalem', value: loading ? '—' : `${totalItems} Adet`, sub: 'Kayıtlı malzeme türü', icon: <Archive size={20} />, color: 'text-metronic-success', bg: 'bg-metronic-success-light dark:bg-metronic-success/10' },
        ].map((stat, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50/50 dark:bg-white/[0.02] text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5">
                <SortableHeader label="Malzeme Bilgisi" column="name" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />
                <SortableHeader label="Kategori" column="category" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />
                <SortableHeader label="Stok Miktarı" column="stock" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="center" />
                <SortableHeader label="Durum" column="status" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="center" />
                <SortableHeader label="Stok Değeri" column="value" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="right" />
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              ...
              {!loading && paginated.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-[13px] font-medium text-slate-600 dark:text-slate-400">{item.category}</span>
                  </td>
                  <td className="px-6 py-4 text-center font-bold text-slate-700 dark:text-slate-300">{item.stock} {item.unit}</td>
                  ...
                  <td className="px-6 py-4 text-right font-black text-metronic-primary">{fmt(item.value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
