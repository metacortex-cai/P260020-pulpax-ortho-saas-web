...
import { ReportsService, CancellationReportItem } from '../../../lib/services/reports.service';
...
export default function CancellationsReportPage() {
  const [data, setData] = useState<CancellationReportItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const result = await ReportsService.getCancellationsReport(startDate, endDate);
        setData(result);
        setCurrentPage(1);
      } catch (err: any) {
  ...
  // Derived KPI values
  const totalCancel = data.reduce((s, i) => s + i.count, 0);
  const totalImpact = data.reduce((s, i) => s + i.impact, 0);

  // Filter → Sort → Paginate
  const filtered = data.filter(item => {
    const ms = item.reason.toLowerCase().includes(searchTerm.toLowerCase());
    const mf =
      filterTrend === 'ALL' ||
      (filterTrend === 'RISING' && item.trend.startsWith('+')) ||
      (filterTrend === 'FALLING' && !item.trend.startsWith('+'));
    return ms && mf;
  });
  ...
  const exportCSV = () => {
    const BOM = '﻿';
    const headers = ['İptal Sebebi', 'İptal Adedi', 'Dağılım (%)', 'Trend', 'Potansiyel Kayıp (₺)'];
    const rows = sorted.map(r => [r.reason, r.count, r.percentage, r.trend, r.impact]);
    const csv = BOM + [headers, ...rows].map(row => row.map(v => `"${String(v).replace(/"/g, '""')}"`).join(';')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'iptal_analizi.csv'; a.click();
    URL.revokeObjectURL(url);
  };

  const trendOpts = [
    { value: 'ALL', label: 'Tüm Sebepler' },
    { value: 'RISING', label: 'Artış Eğilimli' },
    { value: 'FALLING', label: 'Düşüş / Sabit' },
  ];

  return (
    <MetronicLayout title="İptal Randevu Analizi" breadcrumbs={['Raporlar', 'Randevular']}>
      <style>{`@keyframes fadeInDown { from { opacity:0; transform:translateY(-6px); } to { opacity:1; transform:translateY(0); } }`}</style>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Toplam İptal', value: loading ? '—' : `${totalCancel} Adet`, sub: 'Seçili dönem toplam', icon: <UserX size={20} />, color: 'text-metronic-danger', bg: 'bg-red-50 dark:bg-red-500/10' },
          { label: 'İptal Sebebi', value: loading ? '—' : `${data.length} Kategori`, sub: 'Farklı iptal sebebi', icon: <Percent size={20} />, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-500/10' },
          { label: 'Potansiyel Kayıp', value: loading ? '—' : fmt(totalImpact), sub: 'İptal edilen ciro', icon: <XCircle size={20} />, color: 'text-metronic-danger', bg: 'bg-red-50 dark:bg-red-500/10' },
          { label: 'Artış Eğilimli', value: loading ? '—' : `${data.filter(i => i.trend.startsWith('+')).length} Sebep`, sub: 'Yükselen iptal trendi', icon: <RotateCcw size={20} />, color: 'text-metronic-success', bg: 'bg-metronic-success-light dark:bg-metronic-success/10' },
        ].map((stat, i) => (
              ...
              <Dropdown
                trigger={
                  <button className="flex items-center gap-2 h-10 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-lg text-[12px] font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 transition-all">
                    <Filter size={14} /> {trendOpts.find(o => o.value === filterTrend)?.label} <ChevronDown size={12} className="opacity-60" />
                  </button>
                }
              >
                {trendOpts.map(opt => (
                  <DropdownItem
                    key={opt.value}
                    label={opt.label}
                    active={filterTrend === opt.value}
                    onClick={() => { setFilterTrend(opt.value); setCurrentPage(1); }}
                  />
                ))}
              </Dropdown>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50/50 dark:bg-white/[0.02] text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5">
                <SortableHeader label="İptal Sebebi" column="reason" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />
                <SortableHeader label="İptal Adedi" column="count" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="center" />
                <th className="px-6 py-4">Dağılım</th>
                <SortableHeader label="Trend" column="trend" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="center" />
                <SortableHeader label="Potansiyel Kayıp" column="impact" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="right" />
              </tr>
            </thead>
              ...
              {!loading && paginated.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                  <td className="px-6 py-4">
                    <span className="text-[14px] font-bold text-slate-800 dark:text-white leading-tight">{item.reason}</span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3 w-40">
                      <div className="flex-1 h-1.5 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${item.percentage > 30 ? 'bg-metronic-danger' : 'bg-amber-500'}`}
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`text-[12px] font-bold ${item.trend.startsWith('+') ? 'text-metronic-danger' : 'text-metronic-success'}`}>{item.trend}</span>
                  </td>
                  <td className="px-6 py-4 text-right font-black text-slate-700 dark:text-slate-200">{fmt(item.impact)}</td>
                </tr>
              ))}
            </tbody>
          </table>
