...
import { ReportsService, CollectionReportItem } from '../../../lib/services/reports.service';
...
export default function CollectionsReportPage() {
  const [data, setData] = useState<CollectionReportItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const result = await ReportsService.getCollectionsReport(startDate, endDate);
        setData(result);
        setCurrentPage(1);
      } catch (err: any) {
  ...
  // Derived KPI values
  const totalCollection = data.reduce((s, i) => s + i.total, 0);
  const creditCardTotal = data.filter(i => i.method.toLowerCase().includes('kredi')).reduce((s, i) => s + i.total, 0);
  const cashTotal = data.filter(i => i.method.toLowerCase().includes('nakit')).reduce((s, i) => s + i.total, 0);

  const methodIcon = (method: string) => {
    if (method.includes('Nakit')) return <Banknote size={14} className="text-emerald-500" />;
    if (method.includes('Taksitli')) return <CreditCard size={14} className="text-purple-500" />;
    if (method.includes('Kredi')) return <CreditCard size={14} className="text-blue-500" />;
    if (method.includes('Banka') || method.includes('EFT')) return <Building2 size={14} className="text-slate-500" />;
    return <Smartphone size={14} className="text-amber-500" />;
  };

  // Filter → Sort → Paginate
  const filtered = data.filter(item => {
    const ms = item.method.toLowerCase().includes(searchTerm.toLowerCase());
    const mf =
      filterVolume === 'ALL' ||
      (filterVolume === 'HIGH' && item.count >= 100) ||
      (filterVolume === 'LOW' && item.count < 100);
    return ms && mf;
  });
  ...
  const exportCSV = () => {
    const BOM = '﻿';
    const headers = ['Ödeme Yöntemi', 'İşlem Sayısı', 'Dağılım (%)', 'Toplam Tutar (₺)'];
    const rows = sorted.map(r => [r.method, r.count, r.percentage, r.total.toFixed(2)]);
    const csv = BOM + [headers, ...rows].map(row => row.map(v => `"${String(v).replace(/"/g, '""')}"`).join(';')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'tahsilat_raporu.csv'; a.click();
    URL.revokeObjectURL(url);
  };

  const volumeOpts = [
    { value: 'ALL', label: 'Tüm Ödemeler' },
    { value: 'HIGH', label: 'Yüksek Hacimli (≥100)' },
    { value: 'LOW', label: 'Düşük Hacimli (<100)' },
  ];

  return (
    <MetronicLayout title="Tahsilat ve Ödeme Raporu" breadcrumbs={['Raporlar', 'Tahsilatlar']}>
      <style>{`@keyframes fadeInDown { from { opacity:0; transform:translateY(-6px); } to { opacity:1; transform:translateY(0); } }`}</style>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Toplam Tahsilat', value: loading ? '—' : fmt(totalCollection), sub: 'Seçili dönem toplam', icon: <Banknote size={20} />, color: 'text-metronic-success', bg: 'bg-metronic-success-light dark:bg-metronic-success/10' },
          { label: 'Kredi Kartı Payı', value: loading ? '—' : fmt(creditCardTotal), sub: 'Kredi kartı ödemeleri', icon: <CreditCard size={20} />, color: 'text-metronic-primary', bg: 'bg-metronic-primary-light dark:bg-metronic-primary/10' },
          { label: 'Nakit Tahsilat', value: loading ? '—' : fmt(cashTotal), sub: 'Nakit ödemeler', icon: <Banknote size={20} />, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
          { label: 'Ödeme Kanalı', value: loading ? '—' : `${data.length} Yöntem`, sub: 'Aktif ödeme yöntemi', icon: <TrendingUp size={20} />, color: 'text-metronic-danger', bg: 'bg-red-50 dark:bg-red-500/10' },
        ].map((stat, i) => (
              ...
              <Dropdown
                trigger={
                  <button className="flex items-center gap-2 h-10 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-lg text-[12px] font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-50 transition-all">
                    <Filter size={14} /> {volumeOpts.find(o => o.value === filterVolume)?.label} <ChevronDown size={12} className="opacity-60" />
                  </button>
                }
              >
                {volumeOpts.map(opt => (
                  <DropdownItem
                    key={opt.value}
                    label={opt.label}
                    active={filterVolume === opt.value}
                    onClick={() => { setFilterVolume(opt.value); setCurrentPage(1); }}
                  />
                ))}
              </Dropdown>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10">
              <tr className="bg-slate-50/50 dark:bg-white/[0.02] text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-white/5">
                <SortableHeader label="Ödeme Yöntemi" column="method" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />
                <SortableHeader label="İşlem Sayısı" column="count" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="center" />
                <th className="px-6 py-4">Dağılım</th>
                <SortableHeader label="Toplam Tutar" column="total" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} align="right" />
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              ...
              {!loading && !error && paginated.length === 0 && (
                <tr><td colSpan={4} className="py-16 text-center text-slate-400 text-[13px]">Veri bulunamadı.</td></tr>
              )}
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[14px] font-bold text-slate-800 dark:text-white leading-tight">{item.method}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3 w-40">
                      <div className="flex-1 h-1.5 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full rounded-full bg-metronic-primary" style={{ width: `${item.percentage}%` }} />
                      </div>
                      <span className="text-[11px] font-bold text-slate-500">%{item.percentage}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right font-black text-slate-800 dark:text-white">{fmt(item.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
