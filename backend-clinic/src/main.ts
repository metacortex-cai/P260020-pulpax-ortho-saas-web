...
async function bootstrap() {
  ...
  // Hasta dokümanlarını statik olarak servis et (global prefix'ten bağımsız)
  ...
  app.use(
    helmet({
      contentSecurityPolicy: {
        directives: {
          defaultSrc: ["'self'"],
          connectSrc: ["'self'", "https://localhost:7010", "https://localhost:7001", "wss://localhost:7001"],
  ...
  const allowedOrigins = (process.env.CORS_ORIGIN?.split(',') || ['https://localhost:7001']).map(o => o.trim());
  app.enableCors({
  ...
  if (process.env.NODE_ENV !== 'production') {
    const config = new DocumentBuilder()
      .setTitle('Pulpax Clinic API')
      .setDescription('Pulpax Dental Clinic Management System API Specification')
      .setVersion('1.0')
      .addBearerAuth()
