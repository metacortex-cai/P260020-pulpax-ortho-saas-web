...
describe('EmployeesService — İşten Çıkış Hekim Devri', () => {
  ...
  const clinicA = 'clinic-a';
  // Employee (İK kaydı) id'leri:
  const outgoingEmployee = 'emp-outgoing';
  const replacementEmployee = 'emp-replacement';
  const inactiveEmployee = 'emp-inactive';
  const nonDoctorStaff = 'staff-nondoctor';
  // Appointment/Patient/TreatmentItem FK hedefi olan Doctor.id'ler (Employee.doctorId köprüsü):
  const outgoingDoctorId = 'doc-outgoing';
  const replacementDoctorId = 'doc-replacement';
  const inactiveDoctorId = 'doc-inactive';
  ...
  function makeMockTenantClient() {
    return {
          ...
          Promise.resolve(
            employees.find(
              (e) =>
                ...
                (where.doctorId === undefined || e.doctorId === where.doctorId) &&
                (where.clinicId === undefined || e.clinicId === where.clinicId) &&
      ...
      doctor: {
        findUnique: jest.fn(({ where }: any) =>
          Promise.resolve(doctors.find((d) => d.id === where.id || d.emailHash === where.emailHash) || null),
        ),
        update: jest.fn(({ where, data }: any) => {
          const rec = doctors.find((d) => d.id === where.id);
          Object.assign(rec, data);
          return Promise.resolve({ ...rec });
        }),
        create: jest.fn(({ data }: any) => {
          const rec = { id: `doctor-${++idCounter}`, ...data };
          doctors.push(rec);
          return Promise.resolve(rec);
        }),
      },
  ...
  beforeEach(async () => {
    idCounter = 0;
    employees = [
      { id: outgoingEmployee, clinicId: clinicA, doctorId: outgoingDoctorId, isDoctor: true, isActive: true, emailHash: 'hash-outgoing', firstName: 'Outgoing', lastName: 'Doctor' },
      { id: replacementEmployee, clinicId: clinicA, doctorId: replacementDoctorId, isDoctor: true, isActive: true, emailHash: 'hash-replacement', firstName: 'Replacement', lastName: 'Doctor' },
      { id: inactiveEmployee, clinicId: clinicA, doctorId: inactiveDoctorId, isDoctor: true, isActive: false, emailHash: 'hash-inactive', firstName: 'Inactive', lastName: 'Doctor' },
      { id: nonDoctorStaff, clinicId: clinicA, doctorId: null, isDoctor: false, isActive: true, emailHash: 'hash-staff', firstName: 'Non', lastName: 'Doctor' },
    ];
    doctors = [
      { id: outgoingDoctorId, clinicId: clinicA, isActive: true, emailHash: 'hash-outgoing' },
      { id: replacementDoctorId, clinicId: clinicA, isActive: true, emailHash: 'hash-replacement' },
      { id: inactiveDoctorId, clinicId: clinicA, isActive: false, emailHash: 'hash-inactive' },
    ];
    appointments = [];
    patients = [];
    treatmentItems = [];
  ...
  it('devredilecek kaydı olmayan hekim, hedef hekim belirtilmeden doğrudan pasife alınabilir', async () => {
    const updated = await service.deactivate(outgoingEmployee, 'İstifa', clinicA);
  ...
  it('süren randevusu olan hekim, hedef hekim belirtilmeden pasife alınamaz', async () => {
    appointments.push({ id: 'appt-1', clinicId: clinicA, doctorId: outgoingDoctorId, status: 'PLANNED' });

    await expect(service.deactivate(outgoingEmployee, 'İstifa', clinicA)).rejects.toThrow(BadRequestException);
  });

  it('geçerli hedef hekim seçildiğinde randevu, hasta ve tamamlanmamış tedavi kalemleri devredilir', async () => {
    appointments.push(
      { id: 'appt-1', clinicId: clinicA, doctorId: outgoingDoctorId, status: 'PLANNED' },
      { id: 'appt-2', clinicId: clinicA, doctorId: outgoingDoctorId, status: 'COMPLETED' }, // tamamlanmış — devredilmemeli
    );
    patients.push({ id: 'patient-1', clinicId: clinicA, assignedDoctor: outgoingDoctorId });
    treatmentItems.push(
      { id: 'item-1', clinicId: clinicA, doctorId: outgoingDoctorId, status: 'PENDING' },
      { id: 'item-2', clinicId: clinicA, doctorId: outgoingDoctorId, status: 'COMPLETED' }, // tamamlanmış — devredilmemeli
    );

    const impact = await service.getTerminationImpact(outgoingEmployee, clinicA);
    expect(impact).toEqual({ appointmentCount: 1, patientCount: 1, incompleteTreatmentCount: 1, requiresTransfer: true });

    await service.deactivate(outgoingEmployee, 'Emeklilik', clinicA, undefined, replacementEmployee);

    expect(appointments.find((a) => a.id === 'appt-1')!.doctorId).toBe(replacementDoctorId);
    expect(appointments.find((a) => a.id === 'appt-2')!.doctorId).toBe(outgoingDoctorId); // tamamlanmış randevu değişmedi
    expect(patients.find((p) => p.id === 'patient-1')!.assignedDoctor).toBe(replacementDoctorId);
    expect(treatmentItems.find((t) => t.id === 'item-1')!.doctorId).toBe(replacementDoctorId);
    expect(treatmentItems.find((t) => t.id === 'item-2')!.doctorId).toBe(outgoingDoctorId); // tamamlanmış kalem değişmedi

    const outgoing = employees.find((e) => e.id === outgoingEmployee);
  ...
  it('hedef hekim kendisiyle aynı olamaz', async () => {
    ...
    await expect(
      service.deactivate(outgoingEmployee, 'İstifa', clinicA, undefined, outgoingEmployee),
  ...
  it('pasif veya hekim olmayan biri hedef olarak seçilemez', async () => {
    ...
    await expect(
      service.deactivate(outgoingEmployee, 'İstifa', clinicA, undefined, inactiveEmployee),
    ).rejects.toThrow(BadRequestException);
    await expect(
      service.deactivate(outgoingEmployee, 'İstifa', clinicA, undefined, nonDoctorStaff),
    ).rejects.toThrow(BadRequestException);
  });
});
