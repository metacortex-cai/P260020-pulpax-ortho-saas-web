...
export class EmployeesService {
  ...
  async create(createDto: CreateEmployeeDto, clinicId: string) {
    ...
    return this.decryptEmployeeData(employee);
  }
  ...
  async update(id: string, dto: UpdateEmployeeDto, clinicId: string) {
    ...
    return this.decryptEmployeeData(updated);
  }
  ...
  async getTerminationImpact(employeeId: string, clinicId: string) {
    ...
    const [appointmentCount, patientCount, incompleteTreatmentCount] = await Promise.all([
      client.appointment.count({
        where: { clinicId, doctorId: employeeId, status: { notIn: CLOSED_APPOINTMENT_STATUSES } },
      }),
      client.patient.count({ where: { clinicId, assignedDoctor: employeeId } }),
      client.treatmentItem.count({
        where: { doctorId: employeeId, status: { notIn: CLOSED_TREATMENT_ITEM_STATUSES }, plan: { clinicId } },
      }),
    ]);
  ...
  async deactivate(
    ...
    if (existing.isDoctor) {
      ...
      if (impact.requiresTransfer) {
        ...
        const newDoctor = await client.employee.findFirst({
        ...
        await client.$transaction([
          client.appointment.updateMany({
            where: { clinicId, doctorId: id, status: { notIn: CLOSED_APPOINTMENT_STATUSES } },
            data: { doctorId: transferToEmployeeId },
          }),
          client.patient.updateMany({
            where: { clinicId, assignedDoctor: id },
            data: { assignedDoctor: transferToEmployeeId },
          }),
          client.treatmentItem.updateMany({
            where: { doctorId: id, status: { notIn: CLOSED_TREATMENT_ITEM_STATUSES }, plan: { clinicId } },
            data: { doctorId: transferToEmployeeId },
          }),
        ]);
  ...
  async createLeave(dto: CreateLeaveDto, clinicId: string) {
    ...
    const employee = await client.employee.findUnique({ where: { id: dto.employeeId }, select: { isDoctor: true } });
    if (employee?.isDoctor && !dto.force) {
      const conflicting = await client.appointment.findMany({
        where: {
          clinicId,
          doctorId: dto.employeeId,
