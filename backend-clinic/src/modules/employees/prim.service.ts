...
export class PrimService {
   ...
   * Model-2/3 sözleşmelerde lab/tedavi maliyeti düşümü burada (rapor katmanında)
   * netleştirilir — PrimRecord.amount her zaman brüttür.
   */
  async getReport(clinicId: string, filters: { employeeId?: string; from?: string; to?: string }) {
    ...
    for (const bucket of byEmployee.values()) {
      let labCostDeduction = 0;
      let treatmentCostDeduction = 0;

      if (bucket.contract && (bucket.contract.type === 'MODEL_2' || bucket.contract.type === 'MODEL_3')) {
        const labWhere: any = { treatmentItem: { doctorId: bucket.employeeId }, status: { not: 'CANCELLED' } };
        if (filters.from || filters.to) {
          labWhere.createdAt = {};
          if (filters.from) labWhere.createdAt.gte = new Date(filters.from);
          if (filters.to) labWhere.createdAt.lte = new Date(filters.to);
        }
        const labOrders = await client.labOrder.findMany({ where: labWhere });
        labCostDeduction = labOrders.reduce((sum, o) => sum + Number(o.cost), 0);
      }
      ...
      result.push({
        ...
        netConfirmed: bucket.confirmed - labCostDeduction - treatmentCostDeduction,
      });
    }
